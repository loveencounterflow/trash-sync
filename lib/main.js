(function() {
  'use strict';
  var error, exists, log, lstatSync, rpr, runs_as_cli, spawnSync;
  ...
  this.trash = function(path) {
    var command_line, js_source, path_rpr, status, stderr, stdout;
    if (!exists(path)) {
      return 0;
    }
    path_rpr = rpr(path);
    js_source = `import trash from 'trash';await trash(${path_rpr})`;
    ({stdout, stderr, status} = spawnSync(process.execPath, ['-e', js_source], {
      encoding: 'utf-8'
    }));
    //.........................................................................................................
    if (status !== 0) {
  ...
  if (runs_as_cli) {
    (() => {
      ...
      return log(`trash ${path}`, this.trash(path));
    })();
  }
